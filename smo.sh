...
# Function to SSH to ksmo and execute Mplane_pega.sh for Pega RU
configure_pega_ru() {
    # echo "Uploading Mplane_pega.sh to K-releaseSMO..."
    sshpass -p "$SERVER_PASSWORD" scp -o StrictHostKeyChecking=no ~/E2E-network-measurement/tools/Mplane_pega.sh $KSMO_USER@$KSMO_HOST:$KSMO_HOME/
    
    if [ $? -ne 0 ]; then
        echo "Failed to upload Mplane_pega.sh to K-releaseSMO"
        return 1
    fi
    
    sshpass -p "$SERVER_PASSWORD" ssh -o StrictHostKeyChecking=no -o LogLevel=ERROR $KSMO_USER@$KSMO_HOST "cd $KSMO_HOME && TERM=dumb expect -f ./Mplane_pega.sh" < /dev/null
    
    if [ $? -eq 0 ]; then
        echo "Successfully configured Pega RU from K-releaseSMO via Mplane"
    else
        echo "Failed to configure Pega RU from K-releaseSMO via Mplane"
        return 1
    fi
}
